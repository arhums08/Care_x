...
if(isset($_POST['submit'])){
    ...
    $orderno = mt_rand(100000000, 999999999); // Generate a random order number
    
    // Fix the SQL query for tblpatients and tbltestrecord insertion
    $query = "INSERT INTO tblpatients (FullName, MobileNumber, DateOfBirth, hospital, FullAddress, State) 
              VALUES ('$fname', '$mnumber', '$dob', '$hospital', '$address', '$state');";
    $query .= "INSERT INTO tbltestrecord (PatientMobileNumber, VaccinationType, TestTimeSlot, OrderNumber) 
               VALUES ('$mnumber', '$vaccinationtype', '$timeslot', '$orderno');";
    
    // Execute the query using mysqli_multi_query
    $result = mysqli_multi_query($con, $query);
    
    // Check if the query was successful
    if ($result) {
        // Alert with order number
        echo "<script>alert('Your test request submitted successfully. Order number is: " . $orderno . "');</script>";
        echo "<script>window.location.href='new-user-testing.php'</script>"; // Redirect after submission
    } else {
        echo "<script>alert('Something went wrong. Please try again.');</script>";
        echo "<script>window.location.href='new-user-testing.php'</script>"; // Redirect back to form
    }
}
?>
                                ...
                                <div class="card shadow mb-4">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="hospital" name="hospital" required="true">
                                ...
                                <div class="card shadow mb-4">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="vaccinationtype" name="vaccinationtype" required="true">
                                                <option value="">Select</option>
                                                <option value="corona-vac">corona-vac</option>  
                                                <option value="p-fizer">p-fizer</option>
                                                <option value="nova-vax">nova-vax</option>
                                            </select>
                                        </div>
                                        <div class="form-group">
