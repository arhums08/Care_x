...
<?php 
$testid = intval($_GET['tid']);
$query = mysqli_query($con, "SELECT tbltestrecord.*, tblpatients.* FROM tbltestrecord
JOIN tblpatients ON tblpatients.MobileNumber = tbltestrecord.PatientMobileNumber
WHERE tbltestrecord.id = '$testid'");

if (!$query) {
    die("Query Failed: " . mysqli_error($con)); // Debugging: Check if query execution failed.
}
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if (!empty($row['hospital'])) {
                                echo htmlspecialchars($row['hospital']);
                            } else {
                                echo "N/A"; // Display fallback if the hospital is not set.
                            }
                            ?>
                        </td>
                    </tr>
                    <tr>
                        <th>Full Address</th>
                        <td><?php echo htmlspecialchars($row['FullAddress']); ?></td>
                    </tr>
                    <tr>
                        <th>State</th>
